...
> {
    ...
    const columnas = [
      ...
      },
      {
        ...
        selector: (row: any) => {
          ...
          return (
            <Link  data-interception="off" rel="noopener noreferrer" href={row.LinkTitle} target="_blank">
              {file}
            </Link>
          );
        },
    ...
    if (this.state.datefrom) {
      if (query.length === 0) {
        ...
        if (this.state.dateto) {
          query += (" and Created le datetime'" + this.state.dateto + "T23:59:59'");
        }
      }
      else {
  ...
  private async getAIDataTablefiltedate(): Promise<void> {
    ...
    if (minDate) {
      if (query.length === 0) {
        ...
        if (maxDate) {
          query += (" and Created le datetime'" +  this.formatDate(maxDate.toString()) + "T23:59:59'");
        }
      }
      else {
  ...
  private async geTablaReglas(): Promise<void> {
   ...
   // let items: any = [];
    ...
    if (this.props.Tablareglas) {
      try {
        if (query.length > 0) {
          const allItems: any[] = await _sp.web.lists.getById(this.props.Tablareglas.id).items.select(
                "Title",
                "ENTIDAD_FEDERATIVA",
                "CANTIDAD_RECIBIDA",
                "LOTE",
  ...
  finalDataTable = async (): Promise<void> => {
    const result: any = [];
    let resutllote : any=[];
    if(this.state.LoteSearch.length>0 && (this.state.ClaveSearch.length===0 || this.state.NumOrderSearch.length===0)){
      await this.getAIDataTablefiltedate();
      this.state.Remisiones.forEach((remision: {
        ID_x002d_Remision: any; 
      }) => {
    ...
    if (this.state.DatosAI.length > 0) {
      if (this.state.Remisiones.length > 0) {
        resutllote.forEach((datoAI: {
          ...
          const dato = filename.indexOf('PO-') > -1 || filename.indexOf('PO/') > -1;
          if (dato === false) {
    ...
    if (this.state.filteredDataf.length > 0) {
      ...
      let validadornumero = 0;
        ...
        if (ordenes.length > 0) {
          ...
          const ordenesclave = this.findsumm(this.state.filteredDataf, datoAI?.Title);
        ...
        if( (remFilter===undefined || remFilter.length===0)){ 
          ...
          datoAI.stylored = "red";
          messageAi="Error no tiene lectura por la AI en categoría documentos"
         }
        }
        if( (categoriaFilter===undefined || categoriaFilter.length===0)){ 
          if(messageAi.length>0)
          messageAi+=", lectura por remisiones"
         else{
          datoAI.stylored = "red";
          messageAi="Error no tiene lectura por la AI en lecturas por remisiones"
         }
        }
        if(messageAi){ 
          datoAI.stylored = "red";
  ...
  handledelete = async (archivo: any): Promise<void> => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let myList =  spfi().using(SPFx(this.props.context)).web.lists.getById(this.props.Tablareglas.id)
        ...
        myList =  spfi().using(SPFx(this.props.context)).web.lists.getById(this.props.ListCheck.id)
        ...
        myList =  spfi().using(SPFx(this.props.context)).web.lists.getById(this.props.Remisiones.id)
        ...
        myList = spfi().using(SPFx(this.props.context)).web.lists.getById(this.props.DatosAI.id)
        ...
        myList =  spfi().using(SPFx(this.props.context)).web.lists.getById(this.props.Documentos.id)
        itemsToDelete = await myList.items.filter(`Title eq '${columnValue}'`)();
